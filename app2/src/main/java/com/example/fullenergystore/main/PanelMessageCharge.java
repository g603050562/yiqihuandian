...
public class PanelMessageCharge extends Fragment implements OnClickListener,OnScrollListener{
	...
	private HttpPanelIndexCharge th;
	...
	private void handler() {
		panelIndexChargeSuccessHandler = new Handler(){
			...
			public void handleMessage(Message msg) {
				...
				adapter = new SimpleAdapter(getActivity(), getdata(array), R.layout.panel_index_charge_item,new String[]{"name","yuan","xin","date"}, new int[]{R.id.panelIndexChargeItemName,R.id.panelIndexChargeItemYuanID,R.id.panelIndexChargeItemXinID,R.id.panelIndexChargeItemDate});
			...
			public void onRefresh(PullToRefreshBase<ListView> refreshView) {
				...
				if(adapter == null){
					...
					if(PubFunction.isNetworkAvailable(getActivity())){
						init();
						handler();
						th = new HttpPanelIndexCharge(preferences,1,fragement,getActivity());
						th.start();
	...
	private List<Map<String, Object>> getdata(JSONArray array){
		for (int i = 0; i < array.length(); i++) {
			try {
				...
				map.put("yuan", jsonObject.getString("yuan_battery"));
				map.put("xin", jsonObject.getString("new_battery"));
	...
	public void onScrollStateChanged(AbsListView view, int scrollState) {
		if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {  
			if ((view.getLastVisiblePosition() == view.getCount() - 1)&& current_page < count_anli_page) {  
				if(PubFunction.isNetworkAvailable(getActivity())){
					final HttpPanelIndexCharge th = new HttpPanelIndexCharge(preferences,count_anli_page,fragement,getActivity());
					th.start();
					progressDialog.show();
...
class HttpPanelIndexCharge extends Thread{
	...
	public HttpPanelIndexCharge(SharedPreferences preferences,int page,Fragment fragment,Activity activity) {
	...
	public void run() {
		...
		String path = PubFunction.www + "api_business.php/Home/replace_lists";
				...
				if(fragment.getView()!=null){
					if(code.equals("200")){
						if (messageString.equals("请重新登录")) {
							...
							PanelMessageCharge.panelIndexChargeErrorHandler.sendMessage(message);
						}
					}else if(code.equals("100")){
						this.array = jsonObject.getJSONArray("data");
						if(page == 1){
							PanelMessageCharge.panelIndexChargeSuccessHandler.sendMessage(new Message());		
						}else{
							PanelMessageCharge.panelIndexChargeSuccessReHandler.sendMessage(new Message());		
						}
					}else{
						PanelMessageCharge.panelIndexChargeUnknownHandler.sendMessage(new Message());		
					}
				}
			}
		}catch (Exception e) {
