 ...
 */
@EFragment(R.layout.rentcar_area)
public class RentCarArea extends Fragment {
    ...
    @Background
    void getCarProvince(){
        String path = PubFunction.www + "api.php/City/city/"+code;
    ...
    void getTypSuccess(JSONArray jsonArray){
        ...
        rentcar_local_right_list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                RentCarLocal rentCarLocal = new RentCarLocal_();

                Bundle bundle = new Bundle();
                bundle.putString("code1",code);
                bundle.putString("code2",rightList.get(position).get("code").toString());
                rentCarLocal.setArguments(bundle);

                FragmentManager fragmentManager = getFragmentManager();
                FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
                fragmentTransaction.setCustomAnimations(R.anim.in_right, R.anim.out_left);
                fragmentTransaction.replace(R.id.rentcar_panel, rentCarLocal);
                fragmentTransaction.addToBackStack(null);
                fragmentTransaction.commit();
            }
        });
