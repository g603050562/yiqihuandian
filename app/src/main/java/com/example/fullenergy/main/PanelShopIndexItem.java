...
public class PanelShopIndexItem extends Fragment implements OnScrollListener {
	...
	public PanelShopIndexItem(int catid) {
		this.catid = catid;
		if(catid == 9){//9是换电卡
			cat_type = R.layout.panel_shop_index_item_card;
		}else{
			cat_type = R.layout.panel_shop_index_item;
		}
	}
	...
	private void handler() {
		panelShopIndexItemSuccessHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				adapter = new PanelShopIndexItemAdaptr(getActivity(), getdata(array),cat_type,
...
class HttpPanelShopIndexItem extends Thread {
	...
	public HttpPanelShopIndexItem(int catid, SharedPreferences preferences, Handler SuccessHandler,
			Handler ErrorHandler, Handler UnknownHandler, Handler SuccessREHandler,Handler turnToLogin, int page, Fragment fragment,
			Activity activity) {
		this.catid = catid;
...
//			Picasso.with(context).load(PubFunction.www + thumb).memoryPolicy(MemoryPolicy.NO_CACHE, MemoryPolicy.NO_STORE).config(Bitmap.Config.RGB_565).resize(200, 200).centerCrop().into(mHolder.mImageView);
			...
			System.out.println(PubFunction.www + thumb);
